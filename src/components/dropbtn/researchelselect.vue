<template>
    <div class="container" :class="{'mini':size=='mini','user':objtypecode==8||objtypecode==30020}">
        <div class="checkedimg" v-if="value&&value!=''&&showicon!=false">
            ...
            <img v-else-if="filterables" :class="{'borderradius':borderradius}" 
                :src="checkeditem.Icon" alt="">
            <img v-else :class="{'borderradius':borderradius}" 
                :src="'/static/img'+checkeditem.iconUrl" alt="">
        </div>
        <el-select
        :disabled="disabled"
          ...
          :remote-method="function (query) {
           searchlookup(query,objtypecode,name,type,method,parentid);
          }"
        >
            <div v-if="type=='group'">
                <el-option-group
                ...
                label="近期项目">
                <el-option
                    v-for="item in search[name].recentProjects.nodes"
                    :key="item.id"
                    :label="item.name"
                    :value="item.id">
                    <div class="option">
                ...
                label="所有项目">
                <el-option
                    v-for="item in search[name].allProjects.nodes"
                    :key="item.id"
                    :label="item.name"
                    :value="item.id">
                    <div class="option">
            ...
            <div v-if="type=='groups'">
                <el-option-group
                v-for="group in search[name].nodes"
                :key="group.id"
                :label="group.name">
                <el-option
...
export default {
    ...
    created(){
        ...
        if(this.type=='group'){
            ...
            this.$set(this.search,this.name,{
                recentProjects:{
            ...
            this.$set(this.defaultsearch,this.name,this.defaultdata||[])
        }else if(this.type=='groups'){
            this.$set(this.search,this.name,{
                nodes:[{
                    nodes:this.defaultdata||[]
            ...
            this.$set(this.search,this.name,this.defaultdata||[])
            this.$set(this.defaultsearch,this.name,this.defaultdata||[])
        }
